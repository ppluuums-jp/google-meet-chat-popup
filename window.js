...
async function main(tabId) {
    ...
    if (numberOfMessage == 0 && !no_message_flag) {
        ...
        //Execute
        if (back_on_track_flag) {
            message_execute_count = 0;
            i_tmp = 0;
            await getNewMessage("back on track.", count_block, tabId);
            back_on_track_flag = false;
        }
        if (no_message_flag) {
            parent.innerHTML = "";
        ...
        //see if there are diffs of number of blocks
            ...
            await getDiffs("no block diffs.", tabId);
        } else {
            await getNewMessage("There are new messages.", count_block, tabId);
...
async function getNewMessage(text, count_block, tabId) {
...
async function getDiffs(text, tabId) {
